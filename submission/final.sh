...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(echo "$DECODED_TX" | jq -r '.vout[0].value')
...
UTXO_VALUE=$(bitcoin-cli -regtest decoderawtransaction $BASE_TX | jq -r ".vout[$UTXO_VOUT_INDEX].value * 100000000 | floor")
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq '[.vout[].value] | any(. == 0.15)')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq --arg change "$EXPECTED_CHANGE" '[.vout[].value | tostring] | any(. == $change)')
...
SECONDARY_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction $SECONDARY_TX \
  | jq '.vout[0].value * 100000000 | floor')
...
# STUDENT TASK: Create the outputs JSON structure
TTIMELOCK_OUTPUTS="{\"$TIMELOCK_ADDRESS\":$TIMELOCK_BTC}"
